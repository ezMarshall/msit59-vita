...
function generateHours(selectedHour = currentHour) {
    hoursList.innerHTML = '';
...
function generateMinutes(selectedMinute = currentMinute - (currentMinute % 5)) {
    minutesList.innerHTML = '';
...
function updateTimePickerValue() {
    ...
    const selectedMinute = document.querySelector('.minutes .active').textContent;
    timePickerInput.value = selectedHour + ':' + selectedMinute;
}
...
minutesList.addEventListener('click', function (event) {
    const target = event.target;
    if (target.tagName === 'LI') {
        document.querySelector('.minutes .active').classList.remove('active');
        target.classList.add('active');
        updateTimePickerValue();
    }
});

document.querySelector('.date-time-button').addEventListener('click', function () {
    const selectedDate = document.getElementById('datepicker').value;
    const selectedTime = document.getElementById('timepicker').value;
    document.getElementById('timeInput').value = selectedDate + ' ' + selectedTime;
});